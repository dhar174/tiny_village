...
class TinyBrainIO:
    ...
    def load_model(self, model_name="alexredna/TinyLlama-1.1B-Chat-v1.0-reasoning-v2"):
        ...
        if "gguf" in self.model_name.lower():
            self.model = Llama(model_path="./"+self.model_name, n_ctx=self.n_ctx, n_threads=self.n_threads,n_gpu_layers=self.n_gpu_layers, offload_kqv=self.offload_kqv,use_mlock=self.use_mlock)
        else:
            self.model = AutoModelForCausalLM.from_pretrained(
                self.model_name,
                trust_remote_code=True,
                **self.special_args,
                cache_dir="/mnt/d/transformers_cache").to(self.device)
    ...
    def input_to_model(self,prompts, reset_model=True):
        ...
        for text in prompts if isinstance(prompts, list) else [prompts]:
            ...
            if "gguf" in self.model_name.lower():
                ...
                start_time = time.time()
                output = self.model.generate(input_ids, max_new_tokens=20, no_repeat_ngram_size=2)
                print(output)
                end_time = time.time()
