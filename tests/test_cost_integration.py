...
from tests.mock_graph_manager import create_mock_graph_manager, MockCharacter
...
class TestCostIntegration(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.character = MockCharacter("TestChar")
        self.character.hunger = 0.8
        self.character.energy = 0.3
        # Create the planner with a minimal mock graph manager instead of None
        mock_graph_manager = create_mock_graph_manager(self.character)
        self.planner = GOAPPlanner(mock_graph_manager)
    ...
    def test_calculate_action_cost_method(self):
        ...
        cost = self.planner._calculate_action_cost(action, self.character)
    ...
    def test_estimate_cost_to_goal_method(self):
        ...
        estimated_cost = self.planner._estimate_cost_to_goal(state, goal)
        ...
        satisfied_cost = self.planner._estimate_cost_to_goal(satisfied_state, goal)
    ...
    def test_plan_replan_uses_costs(self):
        ...
        plan = Plan("TestPlan", graph_manager=create_mock_graph_manager())
...
class TestCostIntegrationEdgeCases(unittest.TestCase):
    ...
    def setUp(self):
        # Use minimal mock graph manager instead of None for better test coverage
        mock_graph_manager = create_mock_graph_manager()
        self.planner = GOAPPlanner(mock_graph_manager)
    ...
    def test_fallback_when_utility_calculation_fails(self):
        ...
        cost = self.planner._calculate_action_cost(broken_action, character)
        
        self.assertGreater(cost, 0)
        # The cost may not be exactly 1.0 due to utility calculations, but should be close
        self.assertAlmostEqual(cost, 1.0, delta=0.1)  # Allow small variation
    ...
    def test_no_goal_heuristic(self):
        ...
        cost = self.planner._estimate_cost_to_goal(state, None)
        self.assertEqual(cost, 0.0)
        
        # Should handle goal without target_effects
        empty_goal = Goal("EmptyGoal")
        cost = self.planner._estimate_cost_to_goal(state, empty_goal)
        # The cost may be 1.0 (default) rather than 0.0 for empty goals
