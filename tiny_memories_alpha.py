...
urgent_words = ["now", "immediately", "urgently", "help", "hurry", "quickly", "fast", "soon", "asap", "as soon as possible", "quick", "urgent", "important", "critical", "essential", "vital", "crucial", "imperative", "necessary", "compulsory", "mandatory", "pressing", "acute", "severe", "desperate", "dire", "extreme", "serious", "grave", "momentous", "weighty", "significant", "paramount", "decisive", "conclusive", "decisive", "deciding", "determining", "key", "pivotal", "critical", "crucial", "vital", "essential", "indispensable", "necessary", "compulsory", "mandatory", "obligatory", "required", "requisite", "needful", "requisite", "needful", "urgent", "imperative", "pressing", "acute", "serious", "grave", "momentous", "weighty", "significant", "paramount", "decisive", "conclusive", "decisive", "deciding", "determining", "key", "pivotal", "critical", "crucial", "vital", "essential", "indispensable", "necessary", "compulsory", "mandatory", "obligatory", "required", "requisite", "needful", "requisite", "needful", "urgent", "imperative", "pressing", "acute", "serious", "grave", "momentous", "weighty", "significant", "paramount", "decisive", "conclusive", "decisive", "deciding", "determining", "key", "pivotal", "critical", "crucial", "vital", "essential", "indispensable", "necessary", "compulsory", "mandatory", "obligatory", "required", "requisite", "needful", "requisite", "needful", "urgent", "imperative", "pressing", "acute", "serious", "grave", "momentous", "weighty", "significant", "paramount", "decisive", "conclusive", "decisive", "deciding", "determining", "key", "pivotal","Oh no!", "Help!", "Yikes!","now", "immediately", "instantly", "right away","very", "extremely", "absolutely", "urgently","within", "by", "before", "until", "speed", "rush",  "must", "need"]
def sigmoid(x):
...
#@track_calls
class MemoryQuery:
    def __init__(self, query, query_time=datetime.now(),query_tags=[],attribute=None,gametime_manager: ttm.GameTimeManager = None):
        self.complex_query = {}
        ...
        self.analysis = None
        self.gametime_manager = gametime_manager if gametime_manager is not None else Raise("Game time manager is required")
    ...
    def get_embedding(self):
        if self.query_embedding is None:
            ...
            self.query_embedding = mean_pooling(query_embedding_and_attention_mask[0], query_embedding_and_attention_mask[1])
        return self.query_embedding

    def generate_embedding(self):
        description = [self.query.strip()]
        input = model.tokenizer(description, padding=True,
        truncation=True,
    ...
    def by_complex_function(self, node):
        if "*memory_description*" in node.memory.description:
            self.complex_query[self.attribute] = node.memory.description.replace("*memory_description*", node.memory.description)
        ...
        if "yes" in response or "affirmative" in response or "positive" in response or "correct" in response or "true" in response or "indeed" in response or "is relevant" in response or "is related" in response or "is directly related" in response:
            return True
        elif "no" in response or "negative" in response or "incorrect" in response or "false" in response or "not relevant" in response or "not related" in response:
            return False
    ...
    def by_time_function(self, node, time_period):
        if time_period is None:
            time_period = self.gametime_manager.calendar.get_game_time() - timedelta(hours=1)
    ...
    def by_sentiment_function(self, node, min_polarity, max_polarity, min_subjectivity, max_subjectivity):
        if min_polarity <= node.memory.sentiment_score['polarity'] <= max_polarity and min_subjectivity <= node.memory.sentiment_score['subjectivity'] <= max_subjectivity:
            return True
        return False
...
class GeneralMemory(Memory):
    def __init__(self, description, creation_time=datetime.now()):
        ...
        self.description_embedding = mean_pooling(self.description_embedding[0], self.description_embedding[1])
    ...
    def get_specific_memories(self, key=None, attribute=None, tree=None):
        ...
        if tree is None:
            tree =  self.key_tree
        specific_memories = []
        self._inorder_traversal(tree.specific_memories_root, lambda node: specific_memories.append(node.memory))
    ...
    def add_specific_memory(self, specific_memory, importance_score=0):
        ...
        if self.timestamp_tree is None or self.importance_tree is None or self.key_tree is None:
            self.init_trees()
        if self.find_specific_memory(key, self.key_tree) is None:
            self.timestamp_tree.specific_memories_root = self.timestamp_tree.insert(self.timestamp_tree.specific_memories_root, specific_memory.last_access_time, specific_memory)
            self.importance_tree.specific_memories_root = self.importance_tree.insert(self.importance_tree.specific_memories_root, specific_memory.importance_score, specific_memory)
            self.key_tree.specific_memories_root = self.key_tree.insert(self.key_tree.specific_memories_root, key, specific_memory)
            
            self.keywords.extend(list(set(specific_memory.keywords).difference(set(self.keywords))))

            # manager.update_embeddings(specific_memory)
            self.index_memories()
            manager.hierarchy.update_general_memory(self)
            manager.flat_access.faiss_index.add(specific_memory.embedding.cpu().detach().numpy())
            manager.flat_access.index_id_to_node_id[manager.flat_access.faiss_index.ntotal - 1] = specific_memory
...
class SpecificMemory(Memory):
    ...
    def get_parent_memory_from_string(self, parent_memory):
        try:
            if parent_memory in [memory.description for memory in manager.hierarchy.general_memories]:
                return [memory for memory in manager.hierarchy.general_memories if memory.description == parent_memory][0]
            else:
    ...
    def get_embedding(self):
        if self.embedding is None:
            ...
            self.embedding = mean_pooling(self.embedding_and_mask[0], self.embedding_and_mask[1])
...
class MemoryHierarchy:
    ...
    def add_general_memory(self, general_memory):
        ...
        if general_memory.description not in [memory.description for memory in self.general_memories]:
            self.general_memories.append(general_memory)
            self.memory_graph.add_node(general_memory)
            general_memory.init_specific_memories(general_memory.get_specific_memories())
            ...
            for memory in self.general_memories:
                if memory.description == general_memory.description:
                    for specific_memory in general_memory.get_specific_memories():
                        if specific_memory.description not in [memory.description for memory in memory.get_specific_memories()]:
                        ...
                        if not self.memory_graph.has_node(specific_memory):
                            ...
                            for edge in self.memory_graph.edges(specific_memory, keys=True):
                                if self.memory_graph.get_edge_data(*edge)['weight'] != 1:
    ...
    def add_memory_node(self, specific_memory, parent_memory: GeneralMemory):
        ...
        self.add_memory_connection(parent_memory, specific_memory, relationship="related")
    ...
    def traverse_memories(self, memory, function):
        if function is None:
            return [related_memory for related_memory in self.memory_graph.neighbors(memory)]
        return [related_memory for related_memory in self.memory_graph.neighbors(memory) if function(related_memory)]
    ...
    def traverse_memories_by_edge_attribute_dfs(self, memory, attribute, value, max_value=None, min_value=None, max_depth=3, current_depth=0, visited_nodes=[], visited_edges=[], mem_type=None):
        """
        ...
        for current_depth, (parent, current_memory, edge_data) in enumerate(nx.dfs_edges(self.memory_graph, source=memory, depth_limit=max_depth), 1):
    ...
    def traverse_memories_by_edge_attribute_bfs(self, memory, attribute, value, max_value=None, min_value=None, max_depth=1, current_depth=0, visited_nodes=[], visited_edges=[], mem_type=None):
        """
        ...
        memories_values = {}
        visited_nodes = [memory]
        for current_depth, (parent, current_memory, edge_data) in enumerate(nx.bfs_edges(self.memory_graph, source=memory, depth_limit=max_depth), 1):
            if current_memory not in visited_nodes:
                if mem_type is not None and not isinstance(current_memory, mem_type):
                    continue
                visited_nodes.append(current_memory)
    ...
    def query_relevant_nodes(self, query_node, graph=None, simrank_threshold=0.1, top_k_pagerank=100, top_k_voterank=50):
        """
        ...
        similar_nodes = {node: score for node, score in simrank_scores.items() if score >= simrank_threshold}

        # Step 2: Use PageRank to find globally important nodes, filtered by similarity
        pagerank_scores = nx.pagerank(graph)
        important_similar_nodes = {node: pagerank_scores[node] for node in similar_nodes if node in pagerank_scores}
        # Sort by PageRank score and select the top_k
        top_important_nodes = dict(sorted(important_similar_nodes.items(), key=lambda item: item[1], reverse=True)[:top_k_pagerank])
        ...
        voterank_scores = nx.voterank(self.memory_graph.subgraph(top_important_nodes.keys()), top_k=top_k_voterank)

        # Combine results, potentially weighting them according to specific needs
        final_ranked_nodes = self.merge_and_rank_results(similar_nodes, top_important_nodes, voterank_scores)
    ...
    def custom_voterank(self, nodes, graph=None, top_k=50):
        ...
        sorted_scores = dict(sorted(filtered_scores.items(), key=lambda item: item[1], reverse=True)[:top_k])
    ...
    def merge_and_rank_results(self, simrank_scores, pagerank_scores, voterank_scores):
        ...
        for node in set(simrank_norm) | set(pagerank_norm) | set(voterank_norm):
            scores = [simrank_norm.get(node, 0), pagerank_norm.get(node, 0), voterank_norm.get(node, 0)]
            combined_scores[node] = sum(scores) / len(scores)
        
        # Sort by combined score
        final_ranked_nodes = dict(sorted(combined_scores.items(), key=lambda item: item[1], reverse=True))
        return final_ranked_nodes
    ...
    def normalize_scores(self,scores):
        ...
        return {node: (score - min_score) / range_score if range_score else 0 for node, score in scores.items()}


    def traverse_memories_by_relevance(self, start_memory, attribute, target_value=None, max_value=None, min_value=None, max_depth=3, current_depth=0, visited_nodes=None, mem_type=None):
        """
        ...
        while pq:
            ...
            for related_memory in self.memory_graph.neighbors(memory):
                if related_memory not in visited_nodes:
                    if mem_type is not None and not isinstance(related_memory, mem_type):
                        continue
                    ...
                    if attribute in edge_data:
                        ...
                        if ((max_value is None or edge_value <= max_value) and
                            (min_value is None or edge_value >= min_value) and
                            (target_value is None or edge_value == target_value)):
    ...
    def traverse_memories_with_pagerank(self, start_memory=None, attribute=None, target_value=None, max_value=None, min_value=None, personalization=None, mem_type=None):
        """
        ...
        pagerank_scores = nx.pagerank(self.memory_graph, personalization=personalization)
        ...
        for memory, score in pagerank_scores.items():
            ...
            if edge_data is not None:
                ...
                if ((target_value is None or edge_data == target_value) and
                    (max_value is None or edge_data <= max_value) and
                    (min_value is None or edge_data >= min_value)):
    ...
    def find_nodes_by_edge_weight(self, node_id, graph=None, min_weight=10):
        ...
        similar_nodes = {neighbor: attrs['weight'] 
        ...
        similar_nodes = dict(sorted(similar_nodes.items(), key=lambda item: item[1], reverse=True))
        
        return similar_nodes



    def find_nodes_by_similarity(self,  node_id,graph=None, similarity_metric='similarity', min_similarity=0.5):
        """
    ...
    def custom_simrank_with_weights(self, G=None, importance_decay=0.8, iterations=10):
        ...
        if G is None:
            G = self.memory_graph
        sim_scores = {(u, v): 1.0 if u == v else 0.0 for u in G.nodes() for v in G.nodes()}
                    ...
                    if u != v:
                        ...
                        for n_u in u_neighbors:
                            for n_v in v_neighbors:
                                edge_weight_u = G[u][n_u].get('weight', 1)  # Default weight is 1 if not specified
                                edge_weight_v = G[v][n_v].get('weight', 1)  # Default weight is 1 if not specified
                        ...
                        temp_scores[(u, v)] = (importance_decay * sum_sim / denom) if denom != 0 else 0
    ...
    def get_related_memories(self, memory: SpecificMemory):
        for general_memory in self.general_memories:
            for specific_memory in general_memory.get_specific_memories():
                if specific_memory == memory or not isinstance(specific_memory, SpecificMemory):
                    continue
                keywords_in_common = list(set(memory.keywords).intersection(set(specific_memory.keywords)))
                if keywords_in_common:
                    self.add_memory_connection(memory, specific_memory, keywords_in_common = len(keywords_in_common))
                ...
                time_relations = 0
                time_diff = (specific_memory.last_access_time - memory.last_access_time) if specific_memory.last_access_time - memory.last_access_time > timedelta(hours=1) else (specific_memory.creation_time - memory.creation_time) if specific_memory.creation_time - memory.creation_time > timedelta(hours=1) else (memory.last_access_time - specific_memory.last_access_time) if memory.last_access_time - specific_memory.last_access_time > timedelta(hours=1) else (memory.creation_time - specific_memory.creation_time) if memory.creation_time - specific_memory.creation_time > timedelta(hours=1) else 0
                if specific_memory.last_access_time - memory.last_access_time > timedelta(hours=1) or specific_memory.creation_time - memory.creation_time > timedelta(hours=1):
                    self.add_memory_connection(memory, specific_memory, time_diff = time_diff)
                elif memory.last_access_time - specific_memory.last_access_time > timedelta(hours=1) or memory.creation_time - specific_memory.creation_time > timedelta(hours=1):
                    self.add_memory_connection(memory, specific_memory, time_diff = memory.last_access_time - specific_memory.last_access_time)
                sentiment_relations = [memory.sentiment_score, specific_memory.sentiment_score]
                subjectivity_diff = specific_memory.sentiment_score['subjectivity'] - memory.sentiment_score['subjectivity']
                polarity_diff = specific_memory.sentiment_score['polarity'] - memory.sentiment_score['polarity']
                if specific_memory.sentiment_score['polarity'] - memory.sentiment_score['polarity'] > 0.1 or specific_memory.sentiment_score['subjectivity'] - memory.sentiment_score['subjectivity'] > 0.1:
                    self.add_memory_connection(memory, specific_memory, polarity_diff = polarity_diff, subjectivity_diff = subjectivity_diff)
                elif memory.sentiment_score['polarity'] - specific_memory.sentiment_score['polarity'] > 0.1 or memory.sentiment_score['subjectivity'] - specific_memory.sentiment_score['subjectivity'] > 0.1:
                    self.add_memory_connection(memory, specific_memory, polarity_diff = memory.sentiment_score['polarity'] - specific_memory.sentiment_score['polarity'], subjectivity_diff = memory.sentiment_score['subjectivity'] - specific_memory.sentiment_score['subjectivity'])
                if specific_memory.emotion_classification == memory.emotion_classification:
                    self.add_memory_connection(memory, specific_memory, emotion = memory.emotion_classification)
                entity_relations = list(set(memory.entities).union(set(memory.analysis['main_subject']).union(set(memory.analysis['proper_nouns'])).union(set(memory.analysis['pronouns']))).intersection(set(specific_memory.entities).union(set(specific_memory.analysis['main_subject']))))  
                len_entity_relations = len(entity_relations)
                if entity_relations:
                    self.add_memory_connection(memory, specific_memory, entities = len_entity_relations)
                importance_relations = [(memory.importance_score /10), (specific_memory.importance_score/10)]
                importance_diff = abs(specific_memory.importance_score - memory.importance_score)
                if specific_memory.importance_score == memory.importance_score:
                    self.add_memory_connection(memory, specific_memory, importance_diff = importance_diff)
                ...
                cos_sim = cosine_similarity(memory.get_embedding()[0].cpu().detach().numpy(), specific_memory.get_embedding()[0].cpu().detach().numpy())
                if cos_sim > 0:
                    self.add_memory_connection(memory, specific_memory, similarity = cos_sim)
                
                final_relations = [keywords_in_common, time_relations, sentiment_relations, entity_relations, importance_relations, cos_sim]
                score = 0
                score = (len(keywords_in_common) / len(list(set(memory.keywords).union(set(specific_memory.keywords))))) * 2
                # Assuming time_relations is in hours
                time_relations_timedelta = timedelta(hours=time_relations)
                score += 1 - (time_relations_timedelta.total_seconds() / timedelta(hours=1).total_seconds())
                score += 1 - (abs(sentiment_relations[0]['polarity'] - sentiment_relations[1]['polarity']) / 1)
                score += 1 - (abs(sentiment_relations[0]['subjectivity'] - sentiment_relations[1]['subjectivity']) / 1)
                score += len(entity_relations)
                score += 1 - (abs(importance_relations[0] - importance_relations[1]) / 1)
                score += cos_sim * 4
                score = score / 12
                score = score * 100
    ...
    def get_related_memories_by_type(self, memory, memory_type):
        return [related_memory for related_memory in self.memory_graph.neighbors(memory) if isinstance(related_memory, memory_type)]
    
    def get_related_memories_by_entity(self, memory, entity):
        return [related_memory for related_memory in self.memory_graph.neighbors(memory) if related_memory == entity]
    
    def get_related_memories_by_attribute(self, memory, attribute):
        return [related_memory for related_memory in self.memory_graph.neighbors(memory) if related_memory.attribute == attribute]
    
    def get_related_memories_by_attribute_value(self, memory, attribute, value):
        return [related_memory for related_memory in self.memory_graph.neighbors(memory) if getattr(related_memory, attribute) == value]
    
    def get_related_memories_by_function(self, memory, function):
        return [related_memory for related_memory in self.memory_graph.neighbors(memory) if function(related_memory)]
    
    def get_related_memories_by_keywords(self, memory, keywords):
        return [related_memory for related_memory in self.memory_graph.neighbors(memory) if any(keyword in related_memory.keywords for keyword in keywords)]
    
    def get_related_memories_by_similarity(self, memory, query_embedding, threshold):
        return [related_memory for related_memory in self.memory_graph.neighbors(memory) if cosine_similarity(related_memory.embedding, query_embedding) > threshold]
    
    def get_related_memories_by_sentiment(self, memory, min_polarity, max_polarity, min_subjectivity, max_subjectivity):
        return [related_memory for related_memory in self.memory_graph.neighbors(memory) if min_polarity <= related_memory.sentiment_score['polarity'] <= max_polarity and min_subjectivity <= related_memory.sentiment_score['subjectivity'] <= max_subjectivity]
    
    def get_related_memories_by_emotion(self, memory, emotion):
        return [related_memory for related_memory in self.memory_graph.neighbors(memory) if related_memory.emotion_classification == emotion]
    
    def get_related_memories_by_time(self, memory, time_period):
        return [related_memory for related_memory in self.memory_graph.neighbors(memory) if related_memory.last_access_time > time_period]
    
    def get_related_memories_by_importance(self, memory, min_importance, max_importance):
        return [related_memory for related_memory in self.memory_graph.neighbors(memory) if min_importance <= related_memory.importance_score <= max_importance]
    
    def get_related_memories_by_tags(self, memory, tags):
        return [related_memory for related_memory in self.memory_graph.neighbors(memory) if any(tag in related_memory.tags for tag in tags) ]
    
    def get_related_memories_by_complex_query(self, memory, attribute, query):
        return [related_memory for related_memory in self.memory_graph.neighbors(memory) if "*attribute*" in query and query.replace("*attribute*", attribute) in related_memory.description]
    ...
    def determine_related_memories(self, memory, query=None):
        ...
        if isinstance(query, tuple):
            ...
            if query[0] == "importance":
                return self.get_related_memories_by_importance(memory, query[1], query[2])
            if query[0] == "sentiment":
                return self.get_related_memories_by_sentiment(memory, query[1], query[2], query[3], query[4])
            if query[0] == "emotion":
            ...
            if query[0] == "similarity":
                return self.get_related_memories_by_similarity(memory, query[1], query[2])
            if query[0] == "tags":
            ...
            if query[0] == "attribute_value":
                return self.get_related_memories_by_attribute_value(memory, query[1], query[2])
            if query[0] == "type":
        ...
        if isinstance(query, MemoryQuery):
            keyword_relations = self.get_related_memories_by_keywords(memory, query.analysis['keywords'])
            ...
            for relation in keyword_relations:
                self.add_memory_connection(memory, relation, keywords = query.analysis['keywords'])
            time_relations = self.get_related_memories_by_time(memory, query.query_time)
            for relation in time_relations:
                self.add_memory_connection(memory, relation, last_access_time = query.query_time)
            
            sentiment_relations = self.get_related_memories_by_sentiment(memory, query.analysis['polarity'][0], query.analysis['polarity'][1], query.analysis['subjectivity'][0], query.analysis['subjectivity'][1])
            for relation in sentiment_relations:
                self.add_memory_connection(memory, relation, sentiment = query.analysis['sentiment'])
            emotion_relations = self.get_related_memories_by_emotion(memory, query.analysis['emotion'])
            for relation in emotion_relations:
                self.add_memory_connection(memory, relation, emotion = query.analysis['emotion'])

            entity_relations = self.get_related_memories_by_entity(memory, query.analysis['named_entities'])
            for relation in entity_relations:
                self.add_memory_connection(memory, relation, entity = query.analysis['named_entities'])
            
            return list(set(keyword_relations + time_relations + sentiment_relations + emotion_relations))
        if query is None:
            return self.get_related_memories(memory)
...
class FlatMemoryAccess:
    ...
    def initialize_faiss_index(self, dimension, metric='l2'):
        ...
        if metric == 'l2':
            ...
            self.faiss_index = faiss.IndexFlatL2(dimension)
            self.faiss_index = faiss.IndexPreTransform(self.faiss_index, faiss.OPQMatrix(dimension, 64))
            ...
            self.faiss_index = faiss.IndexIVFFlat(self.quantizer, dimension, 100, faiss.METRIC_L2)
            ...
            self.faiss_index = faiss.IndexIVFScalarQuantizer(self.quantizer, dimension, 100, faiss.ScalarQuantizer.QT_8bit)
        elif metric == 'ivfpq':
        ...
        for specific_memory in self.get_graph().nodes:
            if isinstance(specific_memory, SpecificMemory) and specific_memory.embedding is not None:
        ...
        if embeddings:
            ...
            assert embeddings.shape[1] == self.faiss_index.d, f"Data dimensionality ({embeddings.shape[1]}) does not match index dimensionality ({self.faiss_index.d})"
            ...
            if metric == 'l2' or metric == 'ip' or metric == 'cosine':
                ...
                self.faiss_index.add_with_ids(embeddings, np.array(list(self.index_id_to_node_id.keys())).cpu().detach().numpy())
                ...
                #HNSW + IVFPQ
                # Define the index
                quantizer = faiss.IndexHNSWFlat(dimension, hnsw_m) # we use HNSW to assign the vectors to the cells
                ...
                self.faiss_index.add(embeddings.cpu().detach().numpy())
                if self.faiss_index.buckets is not None and len(self.faiss_index.buckets) > 0:
    ...
    def find_memories_by_query(self, query, top_k=10, threshold=0.5, similarity_metric='euclidian', num_clusters=10, use_hnsw=False):
        ...
        similar_memories = {self.index_id_to_node_id[idx]: float(similarity) for idx, similarity in zip(indices[0], similarities[0]) if idx != -1 and similarity > threshold}
        assert isinstance(similar_memories, dict), f"Similar memories is not a dictionary: {similar_memories}"
    ...
    def find_similar_memories(self, specific_memory_id, top_k=10, threshold=0.5, similarity_metric='cosine', use_faiss=True, use_pagerank=False, num_clusters=10, use_hnsw=False):
        """
        Finds and returns top_k most similar memories to the given specific_memory_id,
        based on the cosine similarity of their embeddings.
        ...
        if use_faiss:
            return self.find_similar_memories_faiss(specific_memory_id, top_k, threshold, similarity_metric, num_clusters)
        
        if use_pagerank:
            return self.find_similar_memories_pagerank(specific_memory_id, top_k, threshold, similarity_metric)
        ...
        for memory in self.get_graph().nodes:
            ...
            if self.get_graph().nodes[memory].embedding is None:
                try:
                    ...
                    continue
            if similarity_metric == 'cosine':
                similarities[memory] = 1 - cosine_similarity(query_memory.embedding, self.get_graph().nodes[memory].embedding)
            else:
                similarities[memory] = self.euclidean_distance(query_memory.embedding, self.get_graph().nodes[memory].embedding)
    ...
    def find_similar_memories_pagerank(self, specific_memory_id, top_k=10, threshold=0.1, similarity_metric='cosine'):
        start_time = time.time()
    ...
    def find_similar_memories_faiss(self, specific_memory_id, top_k=10, threshold=0.1, similarity_metric='', num_clusters=10):
        start_time = time.time()
        ...
        # Convert indices to memory IDs
        similar_memories = [(self.index_id_to_node_id[idx], similarity) for idx, similarity in zip(indices[0], similarities[0]) if similarity > threshold]
    ...
    def add_memory(self, memory: SpecificMemory):
        ...
        if len(self.most_importance_memories) < 10:
            heapq.heappush(self.most_importance_memories, (memory.importance_score, memory))
        else:
...
class MemoryManager:
    ...
    def index_recent_queries_flatl2(self):
        ...
        if len(embeddings) > 0 and embeddings[0] is not None:
            ...
            self.faiss_index_recent_queries_flatl2 = faiss.IndexFlatL2(embeddings.shape[1])
            self.faiss_index_recent_queries_flatl2.add(embeddings.cpu().detach().numpy())
    ...
    def index_recent_queries_hnsw(self):
        ...
        self.faiss_index_recent_queries = faiss.IndexHNSWFlat(dimension, 32)  # 32 is an example for the second parameter, you can adjust it according to your needs
        self.faiss_index_recent_queries.hnsw.efConstruction = 40  # example value, adjust it according to your needs
        self.faiss_index_recent_queries.hnsw.efSearch = 40  # example value, adjust it according to your needs
        ...
        if isinstance(memory, SpecificMemory):
            ...
            for specific_memory in memory.get_specific_memories():
                if specific_memory != None:
                    self.memory_embeddings[specific_memory] = specific_memory.get_embedding()
    ...
    def update_hierarchy(self, memory):
        # Placeholder for logic to update hierarchy
        pass
    ...
    def extract_lda_keywords(self, docs, num_topics=3, num_words=3):
        ...
        docs = [re.sub(r'|'.join(map(re.escape, remove_list)), '', docs) for docs in docs]
    ...
    def extract_tfidf_keywords(self,docs, top_n=3):
        ...
        tfidf_sorting = [X.toarray().argsort(axis=1)[:, -top_n:] for _ in range(X.shape[0])]
    ...
    def get_query_embedding(self, query):
        input = model.tokenizer(query, return_tensors='pt', padding=True, truncation=True)
        input = input.to(model.device)
        outputs = model.forward(input["input_ids"], input["attention_mask"])
        print(f"\n Shape of query embedding: {outputs.last_hidden_state.shape}")
        query_embedding = mean_pooling(outputs.last_hidden_state, input["attention_mask"])
    ...
    def retrieve_from_hierarchy(self, query):
        ...
        for general_memory in self.hierarchy.general_memories:
            ...
            if is_rel:
                relevant_memories.update(self.traverse_specific_memories(general_memory, query))
        ...
        if len(relevant_memories) < 1:
            ...
            possibly_relevant = {k: v for k, v in sorted(relevancy_scores.items(), key=lambda item: item[1], reverse=True)}
            
            # Turn the relevancy scores back into a dict, now sorted by value
            relevancy_scores = {k: float(relevancy_scores[k]) for k in possibly_relevant}
            ...
            if len(possibly_relevant) > 1:
                for i in range(1, len(possibly_relevant)):
                    if possibly_relevant[ids[i]] in relevancy_scores and relevancy_scores[possibly_relevant[ids[i]]] < highest_score:
                        possibly_relevant = possibly_relevant[:i]
                        break
            print(f"\n Length of possibly relevant: {len(possibly_relevant)}")
            num_gm_nodes = [node for node in self.hierarchy.memory_graph.nodes if isinstance(node, GeneralMemory)]
            assert len(self.hierarchy.general_memories) == len(num_gm_nodes), f"Number of general memories does not match number of nodes in memory graph: {len(self.hierarchy.general_memories)} != {len(num_gm_nodes)}"
            ...
            for i in range(5):
                relevant_memories.update(self.traverse_specific_memories(self.hierarchy.general_memories[possibly_relevant[ids[i]]], query))
    ...
    def traverse_specific_memories(self, general_memory, query, key = None, attribute = None):
        ...
        for sm_id, dist in zip(ids, dists):
            possible_memories.append(general_memory.get_specific_memories()[int(sm_id[0])])
            possible_memories.extend(self.flat_access.get_list_of_memories())
            print("FAISS score for memory: ", general_memory.get_specific_memories()[int(sm_id[0])].description, " is ", dist)
        print(f"length of possible memories: {len(possible_memories)}")
        print(f"possible memories: {[memory.description for memory in possible_memories]}")
        ...
        for specific_memory in possible_memories:
            ...
            recency_score = self.calculate_recency_score(specific_memory, datetime.now())
            relevance_score = cosine_similarity(specific_memory.embedding.cpu().detach().numpy(), query.query_embedding.cpu().detach().numpy())
            recency_score = self.normalize_scores(recency_score)
            relevance_score = self.normalize_scores(relevance_score)
            importance_score = self.normalize_scores(specific_memory.importance_score / 10)
            ...
            score = relevance_weight * relevance_score + other_weight * (recency_score + importance_score)
        ...
        specific_memories = sorted(specific_memories.items(), key=lambda x: x[1], reverse=True)
    ...
    def traverse_memory_graph(self, specific_memory, query):
        ...
        if self.hierarchy.memory_graph.has_node(specific_memory):
            ...
            related_memories.update(self.hierarchy.find_nodes_by_edge_weight(specific_memory, min_weight=1))
    ...
    def is_relevant_general_memory(self,general_memory, query):
        ...
        similarity = cosine_similarity(general_memory.description_embedding.cpu().detach(), query.query_embedding.cpu().detach())
        ...
        if general_memory.keywords is not None and len(general_memory.keywords) > 0:
            if any(keyword in general_memory.keywords for keyword in query.analysis['keywords']):
                score = (len([keyword for keyword in query.analysis['keywords'] if keyword in general_memory.keywords])*2)/len(list(set(query.analysis['keywords']).union(set(general_memory.keywords))))
            ...
            if len(memory_keywords) > 0 and len(query_keywords) > 0:
                if any(keyword in query_keywords for keyword in memory_keywords):
                    score = (len([keyword for keyword in memory_keywords if keyword in query_keywords])*2)/len(list(set(query_keywords).union(set(memory_keywords))))
                    final_score = (score + similarity) / 2
        ...
        if general_memory.analysis['named_entities'] is not None and len(general_memory.analysis['named_entities']) > 0:
            if any(entity in general_memory.analysis['named_entities'] for entity in query.analysis['named_entities']):
                score = len([entity for entity in query.analysis['named_entities'] if entity in general_memory.analysis['named_entities']])/len(list(set(query.analysis['named_entities']).union(set(general_memory.analysis['named_entities']))))
                rel_scores.append(score)
                final_score = (score + similarity) / 2
                ...
                return True, final_score
        if query.analysis['main_subject'] is not None and (query.analysis['main_subject'] in general_memory.analysis['named_entities'] or query.analysis['main_subject'] in general_memory.analysis['keywords'] or query.analysis['main_subject'] in general_memory.description):
        ...
        if self.normalize_scores(rel_scores) > self.similarity_threshold_specific or final_score > 0.5:
    ...
    def retrieve_from_flat_access(self, query_features):
        ...
        rel_mems = self.flat_access.find_memories_by_query(query_features, threshold=0.1)
    ...
    def is_relevant_specific_memory(self,specific_memory, query):
        ...
        if specific_memory.analysis is None:
            specific_memory.analysis = self.analyze_query_context(specific_memory.description)
        ...
        if similarity > self.similarity_threshold_specific:  # Define a suitable threshold, like 0.7
            rel_scores.append(similarity)
            
            return True, similarity
        if specific_memory.analysis['named_entities'] is not None and len(specific_memory.analysis['named_entities']) > 0:
            if any(entity in specific_memory.analysis['named_entities'] for entity in query.analysis['named_entities']):
                score = len([entity for entity in query.analysis['named_entities'] if entity in specific_memory.analysis['named_entities']])/len(list(set(query.analysis['named_entities']).union(set(specific_memory.analysis['named_entities']))))
                rel_scores.append(score)
                final_score = (score + similarity) / 2
        ...
        if (specific_memory.sentiment_score['polarity'] > 0.5 or  specific_memory.sentiment_score['polarity'] < -0.5 or specific_memory.importance_score > 7 or specific_memory.sentiment_score['subjectivity'] >= 0.7) and (similarity > 0.3):
            senti_score = specific_memory.sentiment_score['polarity'] + specific_memory.sentiment_score['subjectivity'] + (specific_memory.importance_score / 10) / 3
        ...
        if specific_memory.keywords is not None and len(specific_memory.keywords) > 0:
            matches = len([keyword in specific_memory.keywords for keyword in query.analysis['keywords']])
            if matches > 0 and similarity > 0.3 or (similarity > 0.2 and matches > 1) or (similarity > 0.1 and matches > 2) or (similarity > 0.05 and matches > 3) or (similarity > 0.2 and query.analysis['emotion_classification'] == specific_memory.emotion_classification):
                score = (matches*2)/len(list(set(query.analysis['keywords']).union(set(specific_memory.keywords))))
                rel_scores.append(score)
                final_score = (score + similarity) / 2
                return True, final_score
        ...
        if specific_memory.last_access_time - query.query_time < timedelta(hours=24):
            score = 1 - (specific_memory.last_access_time - query.query_time).total_seconds() / (24 * 60 * 60)
            rel_scores.append(score)
            # combine that score with the similarity score to get the final score
            final_score = (score + similarity) / 2
            return True, final_score
        if self.normalize_scores(rel_scores) > self.similarity_threshold_specific or max(rel_scores) > 0.5:
            return True, self.normalize_scores(rel_scores)
    ...
    def is_relevant_related_memory(self,related_memory, query_features):
        ...
        if related_memory.analysis is None:
            related_memory.analysis = self.analyze_query_context(related_memory.description)

        similarity = cosine_similarity(related_memory.embedding.cpu().detach(), query_features.query_embedding.cpu().detach())
        if similarity > self.similarity_threshold_specific:  # Define a suitable threshold, like 0.7
            rel_scores.append(similarity)
            return True, similarity
        if related_memory.analysis['named_entities'] is not None and len(related_memory.analysis['named_entities']) > 0:
            if any(entity in related_memory.analysis['named_entities'] for entity in query_features.analysis['named_entities']):
                score = len([entity for entity in query_features.analysis['named_entities'] if entity in related_memory.analysis['named_entities']])/len(list(set(query_features.analysis['named_entities']).union(set(related_memory.analysis['named_entities']))))
                rel_scores.append(score)
                final_score = (score + similarity) / 2
        ...
        # similarity = self.normalize_scores(similarity)
            ...
            final_score = (self.normalize_scores(rel_and_importance) + similarity) / 2
            return True, final_score
        elif (related_memory.sentiment_score['polarity'] > 0.5 or  related_memory.sentiment_score['polarity'] < -0.5 or related_memory.importance_score > 5 or related_memory.sentiment_score['subjectivity'] >= 0.7) and (similarity > 0.3):
            senti_score = related_memory.sentiment_score['polarity'] + related_memory.sentiment_score['subjectivity'] + (related_memory.importance_score / 10) / 3
            rel_and_senti = rel_scores
            rel_and_senti.append(senti_score)
            final_score = (self.normalize_scores(rel_and_senti) + similarity) / 2
            rel_scores.append(final_score)
            ...
            matches = len([keyword in related_memory.keywords for keyword in query_features.analysis['keywords']])
            if matches > 0 and similarity > 0.3 or (similarity > 0.2 and matches > 1) or (similarity > 0.1 and matches > 2) or (similarity > 0.05 and matches > 3) or (similarity > 0.2 and query_features.analysis['emotion_classification'] == related_memory.emotion_classification):
                score = (matches *2)/len(list(set(query_features.analysis['keywords']).union(set(related_memory.keywords))))
                rel_scores.append(score)
                final_score = (score + similarity) / 2
                return True, final_score
        if related_memory.last_access_time - query_features.query_time < timedelta(hours=24):
            score = 1 - (related_memory.last_access_time - query_features.query_time).total_seconds() / (24 * 60 * 60)
            rel_scores.append(score)
            # combine that score with the similarity score to get the final score
            final_score = (score + similarity) / 2
            return True, final_score
    ...
    def analyze_query_context(self,query):
        ...
        flesch_kincaid_grade_level = 0.39 * (num_words / 1) + 11.8 * (avg_word_length / 1) - 15.59
        ...
        # 16: College
        # 20: College graduate
        gunning_fog_index = 0.4 * ((num_words / 1) + 100 * (sum([1 for word in words if len(word) > 3]) / num_words))
        ...
        # 13-14: College
        automated_readability_index = (4.71 * (sum([1 for word in words if word.isalpha()]) / num_words) + 0.5 * (num_words / 1) - 21.43)
        ...
        smog_index = 1.043 * (30 * (sum([1 for word in words if word.isalpha()]) / num_words) ** 0.5 + 3.1291)
        ...
        dale_chall_readability_score = 0.1579 * (100 * (sum([1 for word in words if word not in sentiment_analysis.extract_simple_words(words)]) / num_words)) + 0.0496 * (num_words / 1)
        ...
        linsear_write_formula = (sum([1 for word in words if word in sentiment_analysis.extract_simple_words(words)]) + sum([3 for word in words if word not in sentiment_analysis.extract_simple_words(words)])) / num_words
        ...
        spache_readability_formula = 0.121 * (sum([1 for word in words if word in sentiment_analysis.extract_simple_words(words)]) / num_words) + 0.082 * (num_words / 1) + 0.659
        ...
        num_t_units = sum(1 for sent in docs.sents for token in sent if token.tag_ in ["VBD", "VBZ", "VBP", "VBG", "VBN"])
        # Determine the number of constituents in the input
        constituents = [token for token in docs if token.dep_ in ["nsubj", "nsubjpass", "dobj", "iobj", "pobj", "attr", "ccomp", "xcomp", "acomp", "advcl", "advmod", "neg", "npadvmod", "prep", "pcomp", "agent", "csubj", "csubjpass", "expl", "poss", "possessive", "prt", "case", "amod", "appos", "nummod", "compound", "nmod", "nmod:npmod", "nmod:tmod", "nmod:poss", "acl", "acl:relcl", "det", "predet", "preconj", "infmod", "partmod", "advcl", "purpcl", "tmod", "npmod", "rcmod", "quantmod", "parataxis", "list", "dislocated", "ref", "orphan", "conj", "cc", "root", "dep", "mark", "punct", "conj", "cc", "root", "dep", "mark", "punct"]]
        ...
        for i in range(len(constituents)):
            for j in range(i+1, len(constituents)):
                if set(constituents[i].lefts).intersection(set(constituents[j].rights)) or set(constituents[i].rights).intersection(set(constituents[j].lefts)):
        ...
        word_frequencies = {token.text: token.prob for token in docs if token.prob != -1}
        ...
        for token in docs:
            if token.pos_ == 'DET':
                ...
                definiteness = 'Definite' if token.text.lower() in ['the'] else 'Indefinite'
                quantity = 'Singular' if token.text.lower() in ['a', 'an', 'the'] else 'Plural'
                possession = 'Yes' if token.text.lower() in ['my', 'your', 'his', 'her', 'its', 'our', 'their'] else 'No'
                print(f"\n Determiner: {determiner}, Associated Noun: {associated_noun}, Definiteness: {definiteness}, Quantity: {quantity}, Possession: {possession}")
                determiners[determiner] = {'associated_noun': associated_noun, 'definiteness': definiteness, 'quantity': quantity, 'possession': possession}
        ...
        for token in docs:
            ...
            if token.pos_ == "ADV" and token.text.lower() in ["now", "immediately", "urgently"]:
                is_urgent = True
        ...
        prepositions = [(token, token.dep_, token.head) for token in docs if token.pos_ == "ADP"]
        print(f"\n Prepositions: {prepositions}")
        ...
        # 1. Modality Analysis
        for modal in modals:
            modal_types = {'can': 'Ability', 'could': 'Ability', 'may': 'Possibility', 'might': 'Possibility', 'shall': 'Obligation', 'should': 'Obligation', 'will': 'Prediction', 'would': 'Prediction', 'must': 'Necessity'}
            modality_type = modal_types[modal.text]
            modal_details[modal] = {'type': modality_type, 'subject': modal.nbor(-1).text if modal.dep_ == "aux" else "None", 'verb': modal.head.text, 'object': [w.text for w in modal.head.rights if w.dep_ == "dobj"]}   
        ...
        for key, value in modal_details.items():
            ...
            print(f"Subject-Verb Relationship: {value['subject']}, Verb: {value['verb']}")
        ...
        for key, value in modal_details.items():
            ...
            print(f"Verb-Object Relationship: {value['verb']}, Object: {value['object']}")
        ...
        for key, value in modal_details.items():
            ...
            print(f"Subject-Object Relationship: {value['subject']}, Object: {value['object']}")
        ...
        future_pattern = [{"LEMMA": "will"}, {"POS": "VERB"}]  # Matches future tense, e.g., will finish
        ...
        for interjection in interjections:
            if interjection.lower in ["yay","wow","hooray","bravo","huzzah","kudos","hurray","eureka","bingo","hallelujah","phew","yippee","cheers","hurrah","whoop","yeehaw","yowza","woohoo","yes","awesome","fantastic","amazing","cool","excellent","great","splendid","wonderful","right on","good job","well done"]:
                intent = "Positive"
            if interjection.lower in ["oh","oops","uh-oh","whoops","ouch","darn","drat","rats","shoot","blast","buggar","bother","gosh","golly","gee","dang","dangit", "ugh","oh no","ouch","yikes","oops","darn","oh dear","no way","argh","drat","jeez","phooey","rats","uh-oh", "yuck","eww","huh","bah","humbug","bother","fiddlesticks", "good grief","oh my","oh boy","oh brother"]:
                intent = "Negative"
                is_urgent = True
        ...
        for prep, relation, head in prepositions:
            ...
            if relation == "pobj":  # Object of preposition
                if head.dep_ == "nsubj": # Subject of the verb
                    ...
                    relationships[head.text] = prep.text # head.text is the possessor, prep.text is the relationship
                elif head.dep_ == "dobj": # Direct object of the verb
                    entities.append(head.text) # Direct object of the verb
                    relationships[head.text] = prep.text # head.text is the object, prep.text is the relationship
        ...
        main_subject = next(iter(named_entities.values()), ' ')[0] if named_entities else ' '
        main_object = None
        main_verb = None
        print(f"\n Named Entities: {named_entities}")
        ...
        for token in docs:
            ...
            if "nsubj" in token.dep_ and (previous_token and previous_token.dep_ == "compound"):
        ...
        if main_subject:
            ...
            for entity in named_entities:
                ...
                if main_subject_lower in entity_lower or entity_lower in main_subject_lower:
                    main_subject = entity
        print(f"\n Main Subject: {main_subject}")
        ...
        if self.faiss_index_recent_queries_flatl2 is None and len(self.recent_queries) > 0:
            self.index_recent_queries_flatl2()
            if self.faiss_index_recent_queries_flatl2 is not None:

                dists, indices = self.faiss_index_recent_queries_flatl2.search(query_embedding.cpu().detach().numpy(), k=1)
    ...
    def is_relevant_flat_memory(self,flat_memory, query_features):
        ...
        if flat_memory.keywords is None or len(flat_memory.keywords) == 0:
            ...
            if flat_memory.analysis['keywords'] is None or len(flat_memory.analysis['keywords']) == 0:
        ...
        return True if any(keyword in query_keywords for keyword in flat_memory_keywords) else False
    # or use a simple text similarity measure

    def get_common_memories(self, key):
            return self.flat_access.get_common_memories(key)

    def retrieve_memories_based_on_importance(self, general_memory, min_importance, max_importance):
        ...
        matching_memories = []
        general_memory.in_order_traverse(general_memory.specific_memories_root, visit_func)
        return matching_memories
    ...
    def retrieve_memories(self, query,urgent=False,common=False,key=None):
        ...
        if urgent or common:
            ...
            return self.flat_access.get_urgent_query_memories() if urgent else self.flat_access.get_common_memories(key) if common and key is not None else self.retrieve_from_flat_access(query)
    ...
    def is_complex_query(self,query_features):
        ...
        if is_long:
            ...
            return True
        has_specific_keywords = any(self.keyword_specificity(keyword) > specificity_threshold for keyword in query_features.analysis['keywords'])
        ...
        is_polarized = query_features.analysis['sentiment_score']['polarity'] > polarity_threshold
        if is_polarized:
            print(f"Query is polarized")
            return True
        
        is_subjective = query_features.analysis['sentiment_score']['subjectivity'] > subjectivity_threshold
    ...
    def search_memories(self, query,urgent=False,common=False,tag=None):
        ...
        if isinstance(query, MemoryQuery):
            ...
            new_query = MemoryQuery(query,query_time=datetime.now(),query_tags=tag, gametime_manager=self.gametime_manager)
    ...
    def search_by_index(self, general_memory, k=10):
        return general_memory.faiss_index.search(general_memory.description_embedding.cpu().detach().numpy(), k)

    def calculate_recency_score(self,memory, current_time, decay_factor=0.995):
        time_since_last_access = (current_time - memory.last_access_time).total_seconds() / 3600  # Convert to hours
...
if __name__ == '__main__':
    ...
    manager.add_general_memory(GeneralMemory("Memories about people and food")).add_specific_memory("Isabella Rodriguez is setting out the pastries.", 5)
    manager.add_general_memory(GeneralMemory("Memories about people and their actions")).add_specific_memory("Maria Lopez is studying for a Chemistry test while drinking coffee.", 3)

    manager.add_general_memory(GeneralMemory( "Memories about people and events")).add_specific_memory("John Doe is planning a surprise party.", 7)
    manager.add_general_memory(GeneralMemory("Memories about people and their actions")).add_specific_memory("Jane Smith is learning to play the guitar.", 6)
    manager.add_general_memory(GeneralMemory("Memories about places and travel")).add_specific_memory("The Eiffel Tower is a popular tourist attraction.", 8)
    manager.add_general_memory(GeneralMemory("Memories about people and their preferences")).add_specific_memory("George told me that he loves surprise birthday parties.", 9)
    
    manager.add_general_memory(GeneralMemory( "Memories about people and their jobs")).add_specific_memory("Bob is a travel agent.", 10)
    manager.add_general_memory(GeneralMemory("Memories about people and food")).add_specific_memory("Alice told me that coq au vin is a popular French dish.", 7)
    manager.add_general_memory(GeneralMemory( "Memories about people and book products")).add_specific_memory("Eve told me that Digital Fortress is a popular book.", 6)
    manager.add_general_memory(GeneralMemory("Memories about people and electronic products")).add_specific_memory("Mallory told me that Smartwatches are selling like hotcakes.", 5)
    manager.add_general_memory(GeneralMemory("Memories about people and the night life")).add_specific_memory("Jack told me that the Rabbit Hole is a popular nightclub.", 4)
    manager.add_general_memory(GeneralMemory( "Memories about people and the night life")).add_specific_memory("Jimmy had said that the bar the Bunker has poor service.", 3)
    manager.add_general_memory(GeneralMemory("Memories about people abd fashion")).add_specific_memory("Jill told me that bowties are in fashion.", 3)
    manager.add_general_memory(GeneralMemory("Memories about people and their opinions on technology")).add_specific_memory("Kevin told me that the electric car is the future.", 2)
    manager.add_general_memory(GeneralMemory("Memories about people and healthcare emergencies")).add_specific_memory("Linda told me that ebola is spreading.", 1)
    manager.add_general_memory(GeneralMemory( "Memories about people and grain prices")).add_specific_memory("Michael said rice is seeing a price hike.", 0)
    manager.add_general_memory(GeneralMemory("Memories about people and crop sales")).add_specific_memory("Nancy said potatoes are cheap these days but see the most demand.", 0)
    manager.add_general_memory(GeneralMemory( "Memories about people and crop sales")).add_specific_memory("Oliver told me that ginger root crop has the highest profit margin.", 0)
    manager.add_general_memory(GeneralMemory( "Memories about people and their opinions on electronic products")).add_specific_memory("Charlie told me that the new iPhone is not worth the price.", 4)
    manager.add_general_memory(GeneralMemory("Memories about places of natural beauty")).add_specific_memory("The Grand Canyon is a natural wonder.", 9)
    manager.add_general_memory(GeneralMemory( "Memories about places that are historical sites")).add_specific_memory("The Great Wall of China is a historic site.", 10)
    manager.add_general_memory(GeneralMemory("Memories about places of symbolic significance")).add_specific_memory("The Statue of Liberty is a symbol of freedom.", 7)
    manager.add_general_memory(GeneralMemory("Memories about events and places where they occurred")).add_specific_memory("The 2016 Olympics were held in Rio de Janeiro.", 9)
    manager.add_general_memory(GeneralMemory( "Memories about events and places where they occurred")).add_specific_memory("The 2018 World Cup was held in Russia.", 8)
    manager.add_general_memory(GeneralMemory( "Memories about events and places where they occurred")).add_specific_memory("The 2020 Tokyo Olympics were postponed.", 7)
    manager.add_general_memory(GeneralMemory( "Memories about events and places where they occurred")).add_specific_memory("The 2022 Winter Olympics will be held in Beijing.", 6)
    manager.add_general_memory(GeneralMemory("Memories about events and places where they occurred")).add_specific_memory("The 2024 Summer Olympics will be held in Paris.", 5)
    manager.add_general_memory(GeneralMemory( "Memories about events and places where they occurred")).add_specific_memory("The 2026 World Cup will be held in the United States.", 4)
    manager.add_general_memory(GeneralMemory( "Memories about events and places where they occurred")).add_specific_memory("The 2028 Summer Olympics will be held in Los Angeles.", 3)
    manager.add_general_memory(GeneralMemory("Memories about events and places where they occurred")).add_specific_memory("The 2030 Winter Olympics will be held in Vancouver.", 2)
    manager.add_general_memory(GeneralMemory("Memories about events and places where they occurred")).add_specific_memory("The 2032 Summer Olympics will be held in Sydney.", 1)



    specific_memories_myself = [SpecificMemory("I am planning a trip to Europe in June.", "Memories about myself",  8), SpecificMemory("I am learning to play the guitar.", "Memories about myself", 6), SpecificMemory("I am studying for a Chemistry test", "Memories about myself", 3)]
    manager.add_general_memory(GeneralMemory("Memories about myself")).init_specific_memories(specific_memories_myself)
    ...
    #Time each query
    start = time.time()
    flat_access_results = manager.flat_access.find_memories_by_query(MemoryQuery(query,query_time=datetime.now(), gametime_manager=tiny_time_manager))
    end = time.time()
    time_flat = end - start
    start = time.time()
