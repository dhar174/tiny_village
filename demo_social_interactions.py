...
def demo_social_interactions():
    ...
    alice = SocialDemoCharacter("Alice", "extraverted")  # Outgoing and friendly
    ...
    interactions = [
        # Alice meets Bob
        (GreetAction(alice, bob, graph_manager=graph_manager), "Alice greets Bob"),

        # Bob responds with a talk
        (TalkAction(bob, alice, graph_manager=graph_manager), "Bob talks to Alice"),

        # Alice shares some news
        (ShareNewsAction(alice, bob, "The village market has fresh apples today!", graph_manager=graph_manager), "Alice shares news with Bob"),

        # Carol joins the conversation
        (GreetAction(carol, alice, graph_manager=graph_manager), "Carol greets Alice"),
        (GreetAction(carol, bob, graph_manager=graph_manager), "Carol greets Bob"),

        # Alice compliments Carol's creativity
        (OfferComplimentAction(alice, carol, "your beautiful artwork", graph_manager=graph_manager), "Alice compliments Carol"),

        # Carol talks with both Alice and Bob
        (TalkAction(carol, alice, graph_manager=graph_manager), "Carol talks with Alice"),
        (TalkAction(carol, bob, graph_manager=graph_manager), "Carol talks with Bob"),

        # Bob overcomes his shyness to compliment Alice
        (OfferComplimentAction(bob, alice, "your friendly personality", graph_manager=graph_manager), "Bob compliments Alice"),

        # More conversations to strengthen bonds
        (TalkAction(alice, bob, graph_manager=graph_manager), "Alice talks more with Bob"),
        (TalkAction(bob, carol, graph_manager=graph_manager), "Bob talks more with Carol"),
    ]
    ...
    print(f"\nGraph contains relationships for: {list(graph_manager.characters.keys())}")
