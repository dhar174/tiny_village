...
class TestEmbeddingModel(unittest.TestCase):
    ...
    def setUp(self, MockAutoModel, MockAutoTokenizer):
        ...
        self.mock_model_instance.to.return_value = self.mock_model_instance # for .to(device)
        
        # Prevent actual model loading during test setup by patching globals
        with patch('tiny_memories.model', None), \\
             patch('tiny_memories.tokenizer', None), \\
             patch('tiny_memories.sa', None), \\
             patch('tiny_memories.manager', None):
    ...
    def test_embedding_model_forward_pass(self):
        ...
        self.embedding_model.model.assert_called_once_with(input_ids=mock_input_ids, attention_mask=mock_attention_mask)
...
class TestSentimentAnalysis(unittest.TestCase):
    ...
    def test_extract_simple_words_string(self):
        ...
        expected_words = ["Hello", "world", "This", "is", "a", "test"] # Assuming default stopwords
        # Filter out stopwords more robustly for comparison
        from nltk.corpus import stopwords
        stop_words = set(stopwords.words('english'))
        expected_words_filtered = [word for word in expected_words if word.lower() not in stop_words and word.lower() not in ENGLISH_STOP_WORDS]
...
class TestMemoryQuery(unittest.TestCase):
    ...
    def test_by_time_function(self):
        # ...existing code...
        self.mq.query_time = "time" # This was comparing string "time" with datetime object.
        # Let's make query_time a datetime object for a more realistic test.
        self.mq.query_time = self.gametime_manager.calendar.get_game_time() - timedelta(minutes=30)
        
        # Mock node.memory.last_access_time to be more recent than query_time - 1 hour
        node.memory.last_access_time = self.gametime_manager.calendar.get_game_time() - timedelta(minutes=10)
        self.assertTrue(self.mq.by_time_function(node, None))

        # Mock node.memory.last_access_time to be older
        node.memory.last_access_time = self.gametime_manager.calendar.get_game_time() - timedelta(hours=2)
    ...
    def test_get_embedding(self, mock_generate_embedding):
        ...
        mock_generate_embedding.return_value = (mock_embedding_val, mock_attention_mask_val)
        ...
        with patch('tiny_memories.mean_pooling') as mock_mean_pooling:
            ...
            mock_mean_pooling.assert_called_once_with(mock_embedding_val, mock_attention_mask_val)
    ...
    def test_generate_embedding(self, mock_global_model):
        ...
        mock_input_dict.to.assert_called_once_with('cpu') # Check that .to(device) was called on tokenized input
    ...
    def test_by_sentiment_function(self):
        ...
        self.assertFalse(self.mq.by_sentiment_function(node, 0.6, 1.0, 0.0, 1.0)) # Polarity too low
        self.assertFalse(self.mq.by_sentiment_function(node, 0.0, 1.0, 0.9, 1.0)) # Subjectivity too low
    ...
    def test_by_similarity_function(self, mock_cosine_similarity):
        ...
        mock_cosine_similarity.return_value = np.array([[0.9]]) # cosine_similarity returns a 2D array
        self.assertTrue(self.mq.by_similarity_function(node, query_embedding, 0.8))
        mock_cosine_similarity.assert_called_once_with(node.memory.embedding, query_embedding)
...
class TestGeneralMemory(unittest.TestCase):
    def setUp(self, mock_model, mock_manager): # Order matters for decorators
        ...
        self.mock_global_manager.analyze_query_context.return_value = {
            ...
            "main_subject": "Subject", "main_verb": "Verb", "main_object": "Object",
            "temporal_expressions": ["today"], "verb_aspects": ["simple_present"]
        }
    ...
    def test_get_embedding(self, mock_model, mock_manager):
        ...
        with patch.object(self.gm, 'generate_embedding', return_value=(MagicMock(), MagicMock())) as mock_gen_emb, \\
             patch('tiny_memories.mean_pooling', return_value=MagicMock(name="PooledEmb")) as mock_mp:
    ...
    def test_generate_embedding(self, mock_model, mock_manager):
        ...
        self.mock_global_model.tokenizer.assert_called_with(
            ...
            is_split_into_words=True, pad_to_multiple_of=8, return_tensors="pt"
        )
    ...
    def test_init_specific_memories(self, MockSpecificMemory, mock_model, mock_manager):
        ...
        sm_instances = [MagicMock(spec=tiny_memories.SpecificMemory) for _ in specific_memories_descs]
        for sm_mock in sm_instances: # Ensure they have necessary attributes if add_specific_memory uses them
    ...
    def test_get_specific_memories_with_items(self, MockSpecificMemory, mock_model, mock_manager):
        ...
        self.gm.add_specific_memory(sm1_desc, importance_score=5) # This will create SpecificMemory instance
    ...
    def test_search_by_key_in_general_memory(self, mock_model, mock_manager):
        ...
        found_mem = self.gm.search_by_key(self.gm.key_tree.specific_memories_root, key_to_find)
        self.assertEqual(found_mem, mock_sm)
        
        self.assertIsNone(self.gm.search_by_key(self.gm.key_tree.specific_memories_root, id(MagicMock())))


    @patch('tiny_memories.SpecificMemory')
    @patch('tiny_memories.faiss.IndexFlatL2')
    def test_add_specific_memory(self, MockFaissIndex, MockSpecificMemory, mock_model, mock_manager):
        ...
        mock_sm_instance.get_embedding.return_value = (mock_embedding_tensor, mock_attention_mask)
        mock_sm_instance.embedding = torch.rand(1, 768) # This is what mean_pooling would produce
        ...
        mock_manager.update_embeddings = MagicMock()
        mock_manager.flat_access = MagicMock()
        mock_manager.flat_access.faiss_index = MagicMock() # This is the global faiss index
        mock_manager.flat_access.index_id_to_node_id = {}


        with patch('tiny_memories.mean_pooling', return_value=mock_sm_instance.embedding) as mock_mp: # if get_embedding is called
        ...
        self.assertEqual(self.gm.key_tree.specific_memories_root.memory, mock_sm_instance)
    ...
    def test_index_memories(self, MockFaissIndexConstructor, mock_model, mock_manager):
        ...
        mock_sm.get_embedding.return_value = (mock_embedding_tensor, mock_attention_mask) # As per current index_memories structure
        ...
        sm1.get_embedding.return_value = (sm1_emb_tensor, sm1_att_mask) # As per current index_memories structure
        ...
        with patch.object(self.gm, 'get_specific_memories', return_value=[sm1, sm2]):
            ...
            MockFaissIndexConstructor.assert_called_once_with(768) # Embedding dim from sm1_emb_tensor.shape[1]
            ...
            mock_faiss_index_instance.add.assert_called_once_with(sm1_emb_tensor.cpu().detach().numpy())
...
class TestSpecificMemory(unittest.TestCase):
    def setUp(self, mock_sa, mock_model, mock_manager): # Order from decorators
        ...
        with patch('tiny_memories.nlp') as mock_spacy_nlp:
            ...
            mock_doc.sents = [mock_sent]
            mock_spacy_nlp.return_value = mock_doc
            self.sm = tiny_memories.SpecificMemory("Test SM Description", self.parent_memory_mock, 5)
    ...
    def test_initialization(self, mock_sa, mock_model, mock_manager):
        ...
        self.mock_global_manager.analyze_query_context.assert_called_once_with("Test SM Description")
        self.assertEqual(self.sm.keywords, ["analyzed_kw"])
        self.assertEqual(self.sm.sentiment_score, {"polarity": 0.1, "subjectivity": 0.2})
        # Test facts extraction
        self.assertTrue(len(self.sm.facts) > 0)
        self.assertIsNotNone(self.sm.facts_embeddings)


    def test_get_parent_memory_from_string_found(self, mock_sa, mock_model, mock_manager):
        ...
        mock_manager.hierarchy.get_general_memory.assert_called_once_with("Existing Parent")
        self.assertEqual(parent, mock_gm_instance)

    def test_get_parent_memory_from_string_not_found(self, mock_sa, mock_model, mock_manager):
        mock_manager.hierarchy = MagicMock()
    ...
    def test_analyze_description(self, mock_sa, mock_model, mock_manager):
        ...
        self.mock_global_manager.analyze_query_context.assert_called_once_with(self.sm.description)
    ...
    def test_update_parent_memory(self, mock_sa, mock_model, mock_manager): # Already exists
    ...
    def test_generate_facts_embeddings(self, mock_mean_pooling, mock_sa, mock_model, mock_manager):
        ...
        self.assertTrue(torch.equal(generated_embeddings[0], mock_pooled_emb1.squeeze(0))) # Squeezed by func
        self.assertTrue(torch.equal(generated_embeddings[1], mock_pooled_emb2.squeeze(0)))
    ...
    def test_extract_facts(self, mock_spacy_nlp, mock_sa, mock_model, mock_manager):
        ...
        self.assertNotIn("This is a good factual sentence.", extracted) # Not entity dense

    def test_add_tag(self, mock_sa, mock_model, mock_manager):
        self.sm.add_tag("new_tag")
        self.assertIn("new_tag", self.sm.tags)
        self.sm.add_tag("new_tag") # Should not add duplicates if it's a set like behavior
        self.assertEqual(self.sm.tags.count("new_tag"), 1) # Assuming tags is a list that prevents duplicates
    ...
    def test_getstate_setstate(self, mock_sa, mock_model, mock_manager):
        ...
        self.assertIn('parent_memory_description', state) # Assuming parent_memory is stored by desc
        ...
        new_sm = tiny_memories.SpecificMemory("Dummy", self.parent_memory_mock, 0) # Create a new instance
